...
def warp_kpts_chd(kpts0, depth0, depth1, height_map0, height_map_info0, T0, T1, K0, K1):
    ...
    Args:
        ...
        height_map_info0 (torch.Tensor): [5,], [cell_size, x_min, y_min, x_max, y_max], 
    ...
    kpts0_height_map_indices = (
        (w_kpts0_ground_xy - torch.tensor([x_min, y_min], device=kpts0.device)) / cell_size
    ).to(torch.int64)
...
# @torch.no_grad()
# def warp_kpts_chd(kpts0, depth0, depth1, height_map0, T0, T1, K0, K1):
#     """ Warp kpts0 from I0 to I1 with depth, K and Rt
#     Compensate for Height Difference using pesudo "height maps" of sythetic images.
#     Also check covisibility and depth consistency.
#     Depth is consistent if relative error < 0.2 (hard-coded).
...
#     )  # (N, L)
#     nonzero_mask = kpts0_height_map != 0  # values in the height map will be zero if not valid
...
#     w_kpts0[~nonzero_mask] = 0  # if height is invalid, warp the point to the left-up corner
